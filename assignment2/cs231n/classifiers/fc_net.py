...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grad_x2, grads['W2'], grads['b2'] = affine_backward(softmax_grad, cache2)
        grads['W2'] += 2 * 0.5 * self.reg * W2
        grad_x, grads['W1'], grads['b1'] = affine_relu_backward(grad_x2, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            ...
            b = np.zeros(shape=hidden_dims[i])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization is None:
                scores, cache_affine_relu = affine_relu_forward(scores, W, b)
                caches["affine_relu" + str(i + 1)] = cache_affine_relu
            else:
                gamma = self.params['gamma' + str(i + 1)]
                beta = self.params['beta' + str(i + 1)]
                if self.normalization == 'batchnorm':

                    scores,cache_affine_bn_relu = affine_bn_relu_forward(scores,W,b,gamma,beta,self.bn_params[i])
                    caches["affine_bn_relu"+str(i + 1)] =cache_affine_bn_relu
                    pass
                if self.normalization == 'layernorm':

                    scores, cache_affine_ln_relu = affine_ln_relu_forward(scores, W, b, gamma, beta, self.bn_params[i])
                    caches["affine_ln_relu" + str(i + 1)] = cache_affine_ln_relu
                    pass
            if self.use_dropout:
        ...
        dout, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(softmax_grad,
                                                                                                     caches["last"])
        grads['W' + str(self.num_layers)] += 2 * 0.5 * self.reg * self.params['W' + str(self.num_layers)]

        for i in range(self.num_layers - 1):
            index = self.num_layers - 1 - i
            if self.use_dropout:
                dout = dropout_backward(dout, caches["dropout" + str(index)])
            if self.normalization is None:
                dout, grads['W' + str(index)], grads['b' + str(index)] = affine_relu_backward(dout, caches["affine_relu" + str(index)])
                grads['W' + str(index)] += 2 * 0.5 * self.reg * self.params['W' + str(index)]
            else:
                if self.normalization == 'batchnorm':
                    dout,grads['W' + str(index)], grads['b' + str(index)],grads['gamma'+str(index)],grads['beta'+str(index)] = affine_bn_relu_backward(dout,caches["affine_bn_relu"+str(index)])
                    grads['W' + str(index)] += 2 * 0.5 * self.reg * self.params['W' + str(index)]
                if self.normalization == 'layernorm':
                    dout, grads['W' + str(index)], grads['b' + str(index)], grads['gamma' + str(index)], grads[
                        'beta' + str(index)] = affine_ln_relu_backward(dout, caches["affine_ln_relu" + str(index)])
                    grads['W' + str(index)] += 2 * 0.5 * self.reg * self.params['W' + str(index)]
...
def affine_bn_relu_backward(dout,cache):
    ...
    dx,dw,db = affine_backward(bn_out,affine_cache)
...
def affine_ln_relu_backward(dout,cache):
    ...
    dx,dw,db = affine_backward(ln_out,affine_cache)
