...
def affine_forward(x, w, b):
    ...
    x_num = x.shape[0]
    x_reshape = np.reshape(x,newshape=(x_num,-1))
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x_norm,gamma,beta,x,sample_mean,sample_var,eps)
        pass
...
def batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = dout.shape
    x_norm,gamma,beta,x,sample_mean,sample_var,eps = cache
    dx_norm = dout*gamma
    dvar = np.sum(dx_norm*(x-sample_mean)*(-0.5)*(sample_var+eps)**(-3/2),axis=0)
    dmean = np.sum(dx_norm*(-1/np.sqrt(sample_var+eps)),axis=0) + dvar*np.sum(-2*(x-sample_mean),axis=0)/N
    dx = dx_norm/np.sqrt(sample_var+eps) + 2*(x-sample_mean)*dvar/N + dmean/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - sample_mean) * (-0.5) * (sample_var + eps) ** (-3 / 2), axis=0)
    dmean = np.sum(dx_norm * (-1 / np.sqrt(sample_var + eps)), axis=0) + dvar * np.sum(-2 * (x - sample_mean),
                                                                                       axis=0) / N
    dx = dx_norm / np.sqrt(sample_var + eps) + 2 * (x - sample_mean) * dvar / N + dmean / N
    dgamma = np.sum(dout * x_norm, axis=0)
    dbeta = np.sum(dout, axis=0)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = x.shape
    sample_mean = np.reshape(np.sum(x,axis=1) /D,(-1,1))

    sample_var =  np.reshape(np.sum(np.square(x - sample_mean), axis=1) / D,(-1,1))
    x_norm = (x - sample_mean) / np.sqrt(sample_var + eps)
    out = x_norm * gamma + beta
    cache = (x_norm, gamma, beta, x, sample_mean, sample_var, eps)
    pass
...
def layernorm_backward(dout, cache):
    ...
    x_norm, gamma, beta, x, sample_mean, sample_var, eps = cache
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - sample_mean) * (-0.5) * (sample_var + eps) ** (-3 / 2), axis=1)
    dvar = np.reshape(dvar,(-1,1))
    dmean = np.sum(dx_norm * (-1 / np.sqrt(sample_var + eps)), axis=1,keepdims=True) + dvar * np.sum(-2 * (x - sample_mean),axis=1,keepdims=True) / D
    dx = dx_norm / np.sqrt(sample_var + eps) + 2 * (x - sample_mean) * dvar / D + dmean / D
    dgamma = np.sum(dout * x_norm, axis=0)
    dbeta = np.sum(dout, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
        for j in range(F):
            for k in range(Hout):
                for t in range(Wout):
                    out[i][j][k][t]=np.sum(w[j]*x_pad[i,:,k*stride:k*stride+HH,t*stride:t*stride+WW])
    out += np.reshape(b,(-1,1,1))
...
def conv_backward_naive(dout, cache):
    ...
    #没啥心情去推了，推导过程可见https://blog.csdn.net/weixin_37251044/article/details/81349287
    dx = np.zeros(x.shape)
    ...
    for k in range(F):  # compute dw
        for n in range(N):  # compute dx_pad
            for i in range(Hout):
                for j in range(Wout):
                    dw[k] += x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride + WW] * dout[n,k,i,j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in  range(Hout):
                for j in range(Wout):
                    pool_district = x[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width]
                    dx[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width] = (pool_district==np.max(pool_district))*dout[n,c,i,j]
